 ...
 * @since 4.0.0
 */
public class SparkplugExpanderMain implements ExtensionMain {

    private static final @NotNull Logger log = LoggerFactory.getLogger(SparkplugExpanderMain.class);
    ...
    private void addPublishModifier() {
        ...
        final SparkplugExpanderInterceptor spInterceptor = new SparkplugExpanderInterceptor();

        initializerRegistry.setClientInitializer(
                (initializerInput, clientContext) -> clientContext.addPublishInboundInterceptor(spInterceptor));
    }
}